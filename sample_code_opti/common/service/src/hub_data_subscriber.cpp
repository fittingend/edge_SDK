...
void HubData_Subscriber::serviceAvailabilityCallback(ara::com::ServiceHandleContainer<Proxy::HandleType> handles, ara::com::FindServiceHandle handler)
{
    (void)handler; // unused
    if (handles.empty()) {
...
void HubData_Subscriber::setupSubscription()
{
    std::lock_guard<std::mutex> lock(m_proxy_mutex);
    if (m_proxy && !m_proxy->hubDataEvent.IsSubscribed()) {
        m_proxy->hubDataEvent.SetReceiveHandler([this]() {
            receivedCallback_hubData();
        });
...
bool HubData_Subscriber::popEvent(std::shared_ptr<hub_data_Objects>& event)
{
    bool result = false;
