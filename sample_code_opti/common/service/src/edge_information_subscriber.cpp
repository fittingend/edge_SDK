...
void EdgeInformation_Subscriber::serviceAvailabilityCallback(ara::com::ServiceHandleContainer<Proxy::HandleType> handles, ara::com::FindServiceHandle handler)
{
    (void)handler; // unused
    if (handles.empty()) {
...
void EdgeInformation_Subscriber::setupSubscription()
{
    std::lock_guard<std::mutex> lock(m_proxy_mutex);
    if (m_proxy && !m_proxy->edgeInformationEvent.IsSubscribed()) {
        m_proxy->edgeInformationEvent.SetReceiveHandler([this]() {
            receivedCallback_edgeInformation();
        });
        m_proxy->edgeInformationEvent.Subscribe(1);
        INFO("EdgeInformation_Subscriber::Subscribe() complete");
    }
}
...
bool EdgeInformation_Subscriber::popEvent(std::shared_ptr<edge_information_Objects>& event)
{
    bool result = false;
