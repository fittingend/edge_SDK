...
void BuildPathTest_Subscriber::serviceAvailabilityCallback(ara::com::ServiceHandleContainer<Proxy::HandleType> handles, ara::com::FindServiceHandle handler)
{
    (void)handler; // unused
    if (handles.empty()) {
...
void BuildPathTest_Subscriber::setupSubscription()
{
    std::lock_guard<std::mutex> lock(m_proxy_mutex);
    if (m_proxy && !m_proxy->buildPathTestEvent.IsSubscribed()) {
        m_proxy->buildPathTestEvent.SetReceiveHandler([this]() {
            receivedCallback_buildPathTest();
        });
        m_proxy->buildPathTestEvent.Subscribe(1);
        INFO("BuildPathTest_Subscriber::Subscribe() complete");
    }
}
...
bool BuildPathTest_Subscriber::popEvent(std::shared_ptr<build_path_test_Objects>& event)
{
    bool result = false;
