...
void BuildPath_Subscriber::serviceAvailabilityCallback(ara::com::ServiceHandleContainer<Proxy::HandleType> handles, ara::com::FindServiceHandle handler)
{
    (void)handler; // unused
    if (handles.empty()) {
...
void BuildPath_Subscriber::setupSubscription()
{
    std::lock_guard<std::mutex> lock(m_proxy_mutex);
    if (m_proxy && !m_proxy->buildPathEvent.IsSubscribed()) {
        m_proxy->buildPathEvent.SetReceiveHandler([this]() {
            receivedCallback_buildPath();
        });
        m_proxy->buildPathEvent.Subscribe(1);
        INFO("BuildPath_Subscriber::Subscribe() complete");
    }
}
...
bool BuildPath_Subscriber::popEvent(std::shared_ptr<build_path_Objects>& event)
{
    bool result = false;
