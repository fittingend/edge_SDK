...
void HubDataImp::ProcessRequests()
{
...
HubData_Provider::HubData_Provider()
{
    DEBUG("object address : %p", static_cast<void*>(this));
}

void HubData_Provider::init(std::string instance)
{
    adcm::Log::Info() << "enter HubData_Provider::init()";
    ...
    m_skeleton = std::make_unique<HubDataImp>(instanceSpec, ara::com::MethodCallProcessingMode::kPoll);
    ...
    adcm::Log::Info() << "exit HubData_Provider::init()";
}

void HubData_Provider::send(hub_data_Objects& data)
{
    try {
        auto allocation = m_skeleton->hubDataEvent.Allocate();
