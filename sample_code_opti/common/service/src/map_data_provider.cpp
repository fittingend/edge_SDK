...
void MapDataImp::ProcessRequests()
{
...
MapData_Provider::MapData_Provider()
{
    DEBUG("object address : %p", static_cast<void*>(this));
}

void MapData_Provider::init(std::string instance)
{
    adcm::Log::Info() << "enter MapData_Provider::init()";
    ...
    m_skeleton = std::make_unique<MapDataImp>(instanceSpec, ara::com::MethodCallProcessingMode::kPoll);
    ...
    adcm::Log::Info() << "exit MapData_Provider::init()";
}

void MapData_Provider::send(map_data_Objects& data)
{
    try {
        auto allocation = m_skeleton->mapDataEvent.Allocate();
        auto l_sampleData = std::move(allocation).Value();
        *l_sampleData = data;
        m_skeleton->mapDataEvent.Send(std::move(l_sampleData));
        // DEBUG("sent");
    }
