    ...
    const double& destination_latitude, const double& destination_longitude, const std::uint8_t& mve_type)
{
    ...
    auto promise = decltype(Skeleton::GetBuildPath(destination_latitude, destination_longitude, mve_type))::PromiseType();
...
void BuildPath_Provider::init(std::string instance)
{
    ...
    m_skeleton = std::make_unique<BuildPathImp>(instanceSpec, ara::com::MethodCallProcessingMode::kPoll);
