...
void RiskAssessmentImp::ProcessRequests()
{
...
RiskAssessment_Provider::RiskAssessment_Provider()
{
    DEBUG("object address : %p", static_cast<void*>(this));
}

void RiskAssessment_Provider::init(std::string instance)
{
    adcm::Log::Info() << "enter RiskAssessment_Provider::init()";
    ...
    m_skeleton = std::make_unique<RiskAssessmentImp>(instanceSpec, ara::com::MethodCallProcessingMode::kPoll);
    ...
    adcm::Log::Info() << "exit RiskAssessment_Provider::init()";
}

void RiskAssessment_Provider::send(risk_assessment_Objects& data)
{
    try {
        auto allocation = m_skeleton->riskAssessmentEvent.Allocate();
        auto l_sampleData = std::move(allocation).Value();
        *l_sampleData = data;
        m_skeleton->riskAssessmentEvent.Send(std::move(l_sampleData));
        // DEBUG("sent");
    }
