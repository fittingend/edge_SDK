...
void ThreadReceiveHubData()
{
    ...
    while (continueExecution)
    {
        ...
        if (hubData_subscriber.waitEvent(100))
        {
            {
                lock_guard<mutex> lock(mtx_data);
                std::shared_ptr<adcm::hub_data_Objects> data;
                while (hubData_subscriber.popEvent(data))
                {
...
void ThreadReceiveWorkInfo()
{
    ...
    while (continueExecution)
    {
        if (workInformation_subscriber.waitEvent(100))
        {
            adcm::Log::Info() << "DataFusion Work Information received";
            std::shared_ptr<adcm::work_information_Objects> data;

            while (workInformation_subscriber.popEvent(data))
            {
...
void ThreadReceiveEdgeInfo()
{
    ...
    while (continueExecution)
    {
        if (edgeInformation_subscriber.waitEvent(100))
        {
            adcm::Log::Info() << "DataFusion Edge Information received";
            std::shared_ptr<adcm::edge_information_Objects> data;
            while (edgeInformation_subscriber.popEvent(data))
            {
                auto state = data->state;
                adcm::Log::Verbose() << "[EdgeInfo] EDGE System State: " << state;
            }
...
int main(int argc, char *argv[])
{
    ...
    adcm::Log::Info() << "SDK release_250325_interface v2.1, AGX Orin common optimization version";
