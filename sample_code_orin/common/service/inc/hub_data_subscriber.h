...
#ifndef __HUB_DATA_SUBSCRIBER_H__
#define __HUB_DATA_SUBSCRIBER_H__
 ...
 */
class HubData_Subscriber
{
    using Proxy = adcm::proxy::hub_dataProxy;
...
public:
    ...
    std::shared_ptr<hub_data_Objects> getEvent();
...
protected:
    ...
    std::queue<std::shared_ptr<hub_data_Objects>> m_Queue_event;
    std::shared_ptr<ara::core::Promise<bool>> m_promise_waitEvent;

    void enQueue(std::shared_ptr<hub_data_Objects> item);
    std::shared_ptr<hub_data_Objects> deQueue();
     ...
     */
    void subscribe_hubData();
};

}  // namespace adcm

#endif  // __HUB_DATA_SUBSCRIBER_H__
