    ...
    namespace etc
    {
        ...
        natsStatus NatsConnManager::NatsConnection()
        {
            ...
            if (s == NATS_OK)
            {
                ...
                {
                    adcm::Log::Info() << "[NATS] Not want subscribe...";
                    adcm::Log::Info() << "[NATS] skip natsConnection_Subscribe()";
                }
                else if (onMsgCb != NULL && subj.size() != 0)
                {
                    adcm::Log::Info() << "[NATS] NATSConnection_Subscribe()";
                    ...
                    for (const char *subject : subj)
                    {
                        ...
                        {
                            s = NATS_ERR;
                            adcm::Log::Info() << "[NATS] Failed to subscribe to subject: " << subject;
                            break;
                        }
                    }
                }
