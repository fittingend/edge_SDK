        ...
        else //이미 obstacle list 존재하는 경우는 obstacle id 비교가 필요하다
        {
            ...
            { 
                for (auto iter1 = obstacle_list.begin(); iter1 != obstacle_list.end();)
                {
                    ...
                    if ((ABS(iter->fused_cuboid_x - iter1->fused_cuboid_x)< 0.01) && (ABS(iter->fused_cuboid_y - iter1->fused_cuboid_y)< 0.01) && (ABS(iter->fused_cuboid_z - iter1->fused_cuboid_z)<0.01))
                    {
