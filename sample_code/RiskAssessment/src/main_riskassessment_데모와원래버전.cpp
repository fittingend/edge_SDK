...
void drawline(doubleVector utm_x, doubleVector utm_y, adcm::riskAssessmentStruct& risk_assessment7, ListVector map_2d)
{
    ...
    for (int count = 0; count < utm_x.size() - 1; count++)
    {
        ...
        for (int x = (int)x_start; x <= maxX; x++)
        {
            if (steep)
            {
                if (map_2d[y][x].road_z == 0) //스캔되지 않은 map 의 index - 노면정보 X 
                {
                    //risk_assessment.push_back({ std::make_pair(utm_x[count], utm_y[count]), std::make_pair(utm_x[count + 1], utm_y[count + 1]), SCENARIO_7, 1 });
                    risk_assessment7.wgs84_xy_start.clear();
                    risk_assessment7.wgs84_xy_end.clear();
                    ...
                    risk_assessment7.wgs84_xy_start.push_back(unscanned_start_path);
                    risk_assessment7.wgs84_xy_end.push_back(unscanned_end_path);
                    risk_assessment7.hazard_class = SCENARIO_7;
                    risk_assessment7.isHarzard = true;
                    adcm::Log::Info() << "Risk assessment generated for X: " << utm_x[count] <<" Y: "<< utm_y[count] << " with flag 1 ";
                    adcm::Log::Info() << "Now break from the loop";
                    break; //한번만 들어가도 for loop break
                }
            }
            else if (map_2d[x][y].road_z == 0) //스캔되지 않은 map 의 index - 노면정보 X 
            {
                //risk_assessment.push_back({ std::make_pair(utm_x[count], utm_y[count]), std::make_pair(utm_x[count + 1], utm_y[count + 1]), SCENARIO_7, 1 });
                risk_assessment7.wgs84_xy_start.clear();
                risk_assessment7.wgs84_xy_end.clear();
...
void GetRiskAssDataFromQueue(std::vector<adcm::risk_assessment_Objects>& dataBatch)
{
    ...
    riskAssDataQueueCondition.wait(lock, []{ return !risk_assessmentDataQueue.empty(); });
...
void ThreadAct1()
{
    ...
    std::vector<adcm::map_2dListVector> map_2d_temp (map_n, adcm::map_2dListVector(map_m, map_2dStruct_init));
    ...
    while (continueExecution) {
        ...
        if(mapData_rxEvent) {
            ...
            while(!mapData_subscriber.isEventQueueEmpty()) {
                ...
                map_2d_temp = data->map_2d;
                obstacle_list_temp = data->obstacle_list;
                auto vehicle_list = data->vehicle_list;

                adcm::Log::Info() << "size of map_2d_temp received: "<<map_2d_temp.size();
...
void ThreadKatech()
{
    ...
    while(continueExecution) 
    {
...

        {
            ...
            adcm::Log::Info() << "=============KATECH: 위험 시나리오1 시작==============";
            ...
            for (auto iter = obstacle_list.begin(); iter != obstacle_list.end(); iter++)
            {
                ...
                {
                    adcm::Log::Info() << "시나리오1-i)장애물 정지 상태 감지 count:" << iter->stop_count;
        ...
        
        {
            ...
            adcm::Log::Info() << "=============KATECH: 위험 시나리오2 시작==============";
...

        {
            ...
            adcm::Log::Info() << "=============KATECH: 위험 시나리오3 시작==============";
...

        {
            ...
            adcm::Log::Info() << "=============KATECH: 위험 시나리오4 시작==============";
...

        {
            ...
            adcm::Log::Info() << "=============KATECH: 위험 시나리오5 시작==============";
            ...
            else
            {
                ...
                {
                    for (auto iter1 = obstacle_pedes_40_50.begin(); iter1 != obstacle_pedes_40_50.end(); iter1++)
                    {
                ...
                //obstacle_pedes_new 에는 동일한 객체만 들어가 있으므로 새 객체와 비교해주기
                for (auto iter = obstacle_pedes_40_50.begin(); iter!= obstacle_pedes_40_50.end(); iter++)
                {
                    for (auto iter1 = obstacle_pedes_repeated.begin(); iter1!= obstacle_pedes_repeated.end(); iter1++)
                    {
                        if (iter->obstacle_id != iter1->obstacle_id)
                        {
                            for (auto iter2 = obstacle_pedes_repeated.begin(); iter2!= obstacle_pedes_repeated.end(); iter2++)
                            {
                                if (iter->obstacle_id == iter2->obstacle_id)
                                {
                                    same_flag=1;
                                    break;
                                }
                    ...
                    if (same_flag ==0)
                    {
                        obstacle_pedes_new.push_back(*iter);
                    }
                    same_flag = 0;
                }
                ...
                {   
                    adcm::Log::Info() << "새로운 객체 등장. 시나리오 5 계속";
                    for (auto iter = obstacle_pedes_initial.begin(); iter!= obstacle_pedes_initial.end(); iter++)
                    {
                        ...
                        if (max_distance < 20)
                        {
                            ...
                            for (auto iter = obstacle_pedes_40_50.begin(); iter != obstacle_pedes_40_50.end(); iter++)
                            {
                                ...
                                if (distance < 30)
                                {
                                    ...
                                    confidence_scenario_5 = 30 / distance * 0.7;
                                    ...
                                    adcm::Log::Info() << "Risk assessment generated: " << iter->obstacle_id << "with confidence:  " << confidence_scenario_5;
...

        {
            ...
            adcm::Log::Info() << "=============KATECH: 위험 시나리오6 시작==============";
...
//시뮬레이션 데이터의 한계로 데모용 추가 코드 작성
            ...
            else
            {
                ...
                {
                    adcm::Log::Info() << "새로운 객체 등장X. 시나리오 6 종료";
                }
                else
                {   
                               ...
                               if (distance < 40)
                                {
                                    ...
                                    adcm::Log::Info() << "Risk assessment generated: " << iter->obstacle_id << "with confidence:  " << confidence_scenario_6;
#ifndef DEMO
                                }
#endif
                            }
        ...
        {
            //=====시나리오 #7. 미개척 지역 주행환경======
            adcm::Log::Info() << "=============KATECH: 위험 시나리오7 시작==============";
            adcm::riskAssessmentStruct riskAssessment7;
            drawline(utm_x, utm_y, riskAssessment7, map_2d);
            riskAssessment.riskAssessmentList.push_back(riskAssessment7);
            adcm::Log::Info() << "Risk assessment generated: " << riskAssessment7.wgs84_xy_start[0].x << " , " <<riskAssessment7.wgs84_xy_start[0].y;

        }
        
		adcm::Log::Info() << "build riskAssessment data";
