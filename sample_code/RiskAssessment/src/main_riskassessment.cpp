...
float getDistance_LinearTrajectory(adcm::obstacleListStruct obstacle, doubleVector position_x, doubleVector position_y)
{
    float distance = 0;
    for (int count = 0; count < position_x.size(); count++)
    {
        float x_start = position_x[count];
        float x_end = position_x[count + 1];
        float y_start = position_y[count];
        float y_end = position_y[count + 1];
...
void drawline(doubleVector position_x, doubleVector position_y, std::vector<adcm::map_2dListVector> &map_2d, adcm::risk_assessment_Objects &riskAssessment)
{
...
namespace RiskAssessment
{
        ...
        while (continueExecution)
        {
            ...
            if (mapData_rxEvent)
            {
                ...
                while (!mapData_subscriber.isEventQueueEmpty())
                {
                    ...
                    auto obstacle_list_temp = data->obstacle_list;
            ...
            /*if (buildPathTest_rxEvent)
            {
                ...
                while (!buildPathTest_subscriber.isEventQueueEmpty())
                {
                    ...
                    utm_x = data->utm_x;
                    utm_y = data->utm_y;
                    gpsToMapcoordinate(utm_x, utm_y);
            ...
            {
                {
                    ...
                    {
                        distance_scenario_1 = getDistance_LinearTrajectory(*iter, position_x, position_y);
...

                {
                    ...
                    {
                        distance_scenario_2 = getDistance_LinearTrajectory(*iter, position_x, position_y);
    ...
    #define TTC_THRESHOLD 100
...

                {
                    ...
                    {
                        distance = getDistance_LinearTrajectory(*iter, position_x, position_y);
    ...
    */
...

                {
                    ...
                    {
                        drawline(position_x, position_y, map_2d, riskAssessment);
                    }
...
int main(int argc, char *argv[])
{
    ...
    if(!RiskAssessment::RegisterSigTermHandler()) {
    ...
    thread_list.push_back(std::thread(RiskAssessment::ThreadReceiveMapData));
    thread_list.push_back(std::thread(RiskAssessment::ThreadReceiveBuildPathTest));
    thread_list.push_back(std::thread(RiskAssessment::ThreadReceiveBuildPath));
    thread_list.push_back(std::thread(RiskAssessment::ThreadMonitor));
    thread_list.push_back(std::thread(RiskAssessment::ThreadKatech));
