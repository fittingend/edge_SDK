...
//        adcm::Log::Info() << "drawline test 1: "<< utm_x.size();
        ...
        for (int x = (int)x_start; x <= maxX; x++)
        {
            ...
            else if (map_2d[x][y].road_z !=1) //스캔되지 않은 map 의 index - 노면정보 X 
            {
                ...
                riskAssessment7.hazard_class = SCENARIO_7;
                riskAssessment7.isHarzard = true;
                adcm::Log::Info() << "Risk assessment generated for for #7 is X: " << riskAssessment7.wgs84_xy_start[0].x <<" Y: "<< riskAssessment7.wgs84_xy_start[0].y << " with flag" << riskAssessment7.isHarzard;
                riskAssessment.riskAssessmentList.push_back(riskAssessment7);
                break;
            }
    ...
    while (continueExecution) {
        ...
        if(mapData_rxEvent) {
            ...
            while(!mapData_subscriber.isEventQueueEmpty()) {
                ...
                adcm::Log::Info() << "size of map_2d received: "<<map_2d.size();
...
//        map_2d = map_2d_temp;
        ...
        adcm::Log::Info() << "map_2d size (katech)" << map_2d.size();
...
//시뮬레이션 데이터의 한계로 데모용 추가 코드 작성
        ...
        {
            adcm::Log::Info() << "what's inside? "<< iter->obstacle_id << " hazard class:" << iter->hazard_class << " confidence:" << iter->confidence;
        }
        riskAssessment_provider.send(riskAssessment);
        riskAssessment.riskAssessmentList.clear();
