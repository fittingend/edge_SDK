...
#ifndef __BUILD_PATH_TEST_SUBSCRIBER_H__
#define __BUILD_PATH_TEST_SUBSCRIBER_H__
 ...
 */
class BuildPathTest_Subscriber
{
    using Proxy = adcm::proxy::build_path_testProxy;

public:
    BuildPathTest_Subscriber();
    ...
    std::shared_ptr<build_path_test_Objects> getEvent();
     ...
     */
    void receivedCallback_buildPathTest();
...
protected:
    ...
    std::queue<std::shared_ptr<build_path_test_Objects>> m_Queue_event;
    std::shared_ptr<ara::core::Promise<bool>> m_promise_waitEvent;

    void enQueue(std::shared_ptr<build_path_test_Objects> item);
    std::shared_ptr<build_path_test_Objects> deQueue();
     ...
     */
    void subscribe_buildPathTest();
};

}  // namespace adcm

#endif  // __BUILD_PATH_TEST_SUBSCRIBER_H__
