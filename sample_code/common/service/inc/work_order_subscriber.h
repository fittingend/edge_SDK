...
#ifndef __WORK_ORDER_SUBSCRIBER_H__
#define __WORK_ORDER_SUBSCRIBER_H__
 ...
 */
class WorkOrder_Subscriber
{
    using Proxy = adcm::proxy::work_orderProxy;

public:
    WorkOrder_Subscriber();
    ...
    std::shared_ptr<work_order_Objects> getEvent();
     ...
     */
    void receivedCallback_workOrder();
...
protected:
    ...
    std::queue<std::shared_ptr<work_order_Objects>> m_Queue_event;
    std::shared_ptr<ara::core::Promise<bool>> m_promise_waitEvent;

    void enQueue(std::shared_ptr<work_order_Objects> item);
    std::shared_ptr<work_order_Objects> deQueue();
     ...
     */
    void subscribe_workOrder();
};

}  // namespace adcm

#endif  // __WORK_ORDER_SUBSCRIBER_H__
