...
HubData_Subscriber::HubData_Subscriber()
{
    m_proxy = nullptr;
...
void HubData_Subscriber::init(std::string instance)
{
    adcm::Log::Info() << "enter HubData_Subscriber::init()";
    ...
    [this](ara::com::ServiceHandleContainer<Proxy::HandleType> handles, ara::com::FindServiceHandle handler) {
        HubData_Subscriber::serviceAvailabilityCallback(std::move(handles), handler);
    },
    instanceIDs[0]);
    adcm::Log::Info() << "exit HubData_Subscriber::init()";
}

void HubData_Subscriber::subscribe_hubData()
{
    if(!m_proxy->hubDataEvent.IsSubscribed()) {
        // m_proxy got initialized via callback.
        m_proxy->hubDataEvent.SetReceiveHandler([this]() {
            HubData_Subscriber::receivedCallback_hubData();
        });
        // subscribe to event
        m_proxy->hubDataEvent.Subscribe(1);
        INFO("HubData_Subscriber::subscribe_hubData() complete");
    }
}

void HubData_Subscriber::serviceAvailabilityCallback(ara::com::ServiceHandleContainer<Proxy::HandleType> handles,
        ara::com::FindServiceHandle handler)
{
    ...
    if(handles.size() > 0) {
        ...
        if(nullptr == m_proxy) {
            ...
            INFO("subscribe hubData service");
            subscribe_hubData();
        }
    }
}

void HubData_Subscriber::receivedCallback_hubData()
{
...
bool HubData_Subscriber::waitEvent(int deadLine)
{
...
void HubData_Subscriber::enQueue(std::shared_ptr<hub_data_Objects> item)
{
    std::lock_guard<std::mutex> guard(m_Mutex_eventQueue);
...
std::shared_ptr<hub_data_Objects> HubData_Subscriber::deQueue()
{
    std::shared_ptr<hub_data_Objects> temp;
    std::lock_guard<std::mutex> guard(m_Mutex_eventQueue);
...
std::shared_ptr<hub_data_Objects> HubData_Subscriber::getEvent()
{
    std::shared_ptr<hub_data_Objects> result;
...
bool HubData_Subscriber::isEventQueueEmpty()
{
    std::lock_guard<std::mutex> guard(m_Mutex_eventQueue);
