...
#include "adcm/work_order_proxy.h"
...
WorkOrder_Subscriber::WorkOrder_Subscriber()
{
    m_proxy = nullptr;
...
void WorkOrder_Subscriber::init(std::string instance)
{
    adcm::Log::Info() << "enter WorkOrder_Subscriber::init()";
    ...
    [this](ara::com::ServiceHandleContainer<Proxy::HandleType> handles, ara::com::FindServiceHandle handler) {
        WorkOrder_Subscriber::serviceAvailabilityCallback(std::move(handles), handler);
    },
    instanceIDs[0]);
    adcm::Log::Info() << "exit WorkOrder_Subscriber::init()";
}

void WorkOrder_Subscriber::subscribe_workOrder()
{
    if(!m_proxy->workOrderEvent.IsSubscribed()) {
        // m_proxy got initialized via callback.
        m_proxy->workOrderEvent.SetReceiveHandler([this]() {
            WorkOrder_Subscriber::receivedCallback_workOrder();
        });
        // subscribe to event
        m_proxy->workOrderEvent.Subscribe(1);
        INFO("WorkOrder_Subscriber::subscribe_workOrder() complete");
    }
}

void WorkOrder_Subscriber::serviceAvailabilityCallback(ara::com::ServiceHandleContainer<Proxy::HandleType> handles,
        ara::com::FindServiceHandle handler)
{
    ...
    if(handles.size() > 0) {
        ...
        if(nullptr == m_proxy) {
            ...
            INFO("subscribe workOrder service");
            subscribe_workOrder();
        }
    }
}

void WorkOrder_Subscriber::receivedCallback_workOrder()
{
    // execute callback for every samples in the context of GetNewSamples
    std::shared_ptr<work_order_Objects> temp;
    auto callback = [&temp](auto sample) {
        DEBUG("Callback: workOrderEvent ");
        temp = std::make_shared<work_order_Objects>(*sample);
    };
    m_proxy->workOrderEvent.GetNewSamples(callback);
    this->enQueue(temp);
}

int WorkOrder_Subscriber::getQueueSize()
{
    std::lock_guard<std::mutex> guard(m_Mutex_eventQueue);
    return static_cast<int>(m_Queue_event.size());
}

auto WorkOrder_Subscriber::setTrigger()
{
    std::lock_guard<std::mutex> guard(m_Mutex_eventQueue);
...
bool WorkOrder_Subscriber::waitEvent(int deadLine)
{
...
void WorkOrder_Subscriber::enQueue(std::shared_ptr<work_order_Objects> item)
{
    std::lock_guard<std::mutex> guard(m_Mutex_eventQueue);
...
std::shared_ptr<work_order_Objects> WorkOrder_Subscriber::deQueue()
{
    std::shared_ptr<work_order_Objects> temp;
    std::lock_guard<std::mutex> guard(m_Mutex_eventQueue);
...
std::shared_ptr<work_order_Objects> WorkOrder_Subscriber::getEvent()
{
    std::shared_ptr<work_order_Objects> result;
...
bool WorkOrder_Subscriber::isEventQueueEmpty()
{
    std::lock_guard<std::mutex> guard(m_Mutex_eventQueue);
