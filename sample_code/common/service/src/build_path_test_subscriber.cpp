...
#include "adcm/build_path_test_proxy.h"
...
BuildPathTest_Subscriber::BuildPathTest_Subscriber()
{
    m_proxy = nullptr;
...
void BuildPathTest_Subscriber::init(std::string instance)
{
    adcm::Log::Info() << "enter BuildPathTest_Subscriber::init()";
    ...
    [this](ara::com::ServiceHandleContainer<Proxy::HandleType> handles, ara::com::FindServiceHandle handler) {
        BuildPathTest_Subscriber::serviceAvailabilityCallback(std::move(handles), handler);
    },
    instanceIDs[0]);
    adcm::Log::Info() << "exit BuildPathTest_Subscriber::init()";
}

void BuildPathTest_Subscriber::subscribe_buildPathTest()
{
    if(!m_proxy->buildPathTestEvent.IsSubscribed()) {
        // m_proxy got initialized via callback.
        m_proxy->buildPathTestEvent.SetReceiveHandler([this]() {
            BuildPathTest_Subscriber::receivedCallback_buildPathTest();
        });
        // subscribe to event
        m_proxy->buildPathTestEvent.Subscribe(1);
        INFO("BuildPathTest_Subscriber::subscribe_buildPathTest() complete");
    }
}

void BuildPathTest_Subscriber::serviceAvailabilityCallback(ara::com::ServiceHandleContainer<Proxy::HandleType> handles,
        ara::com::FindServiceHandle handler)
{
    ...
    if(handles.size() > 0) {
        ...
        if(nullptr == m_proxy) {
            ...
            INFO("subscribe buildPathTest service");
            subscribe_buildPathTest();
        }
    }
}

void BuildPathTest_Subscriber::receivedCallback_buildPathTest()
{
    // execute callback for every samples in the context of GetNewSamples
    std::shared_ptr<build_path_test_Objects> temp;
    auto callback = [&temp](auto sample) {
        // DEBUG("Callback: buildPathTestEvent ");
        temp = std::make_shared<build_path_test_Objects>(*sample);
    };
    m_proxy->buildPathTestEvent.GetNewSamples(callback);
    this->enQueue(temp);
}

int BuildPathTest_Subscriber::getQueueSize()
{
    std::lock_guard<std::mutex> guard(m_Mutex_eventQueue);
    return static_cast<int>(m_Queue_event.size());
}

auto BuildPathTest_Subscriber::setTrigger()
{
    std::lock_guard<std::mutex> guard(m_Mutex_eventQueue);
...
bool BuildPathTest_Subscriber::waitEvent(int deadLine)
{
...
void BuildPathTest_Subscriber::enQueue(std::shared_ptr<build_path_test_Objects> item)
{
    std::lock_guard<std::mutex> guard(m_Mutex_eventQueue);
...
std::shared_ptr<build_path_test_Objects> BuildPathTest_Subscriber::deQueue()
{
    std::shared_ptr<build_path_test_Objects> temp;
    std::lock_guard<std::mutex> guard(m_Mutex_eventQueue);
...
std::shared_ptr<build_path_test_Objects> BuildPathTest_Subscriber::getEvent()
{
    std::shared_ptr<build_path_test_Objects> result;
...
bool BuildPathTest_Subscriber::isEventQueueEmpty()
{
    std::lock_guard<std::mutex> guard(m_Mutex_eventQueue);
