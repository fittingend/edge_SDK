...
void WorkOrderImp::ProcessRequests()
{
    while(!m_finished) {
...
WorkOrder_Provider::WorkOrder_Provider()
{
...
void WorkOrder_Provider::init(std::string instance)
{
    adcm::Log::Info() << "enter WorkOrder_Provider::init()";
    ...
    m_skeleton = new WorkOrderImp(instanceSpec, ara::com::MethodCallProcessingMode::kPoll);
    ...
    adcm::Log::Info() << "exit WorkOrder_Provider::init()";
}

void WorkOrder_Provider::send(work_order_Objects& data)
{
    try {
        auto allocation = m_skeleton->workOrderEvent.Allocate();
        auto l_sampleData = std::move(allocation).Value();
        *l_sampleData = data;
        m_skeleton->workOrderEvent.Send(std::move(l_sampleData));
        DEBUG("sent");
